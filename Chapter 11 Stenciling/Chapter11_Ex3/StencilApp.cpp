...
void StencilApp::Draw(const GameTimer& gt)
{
	...
	//!? look here: we make it so that when we draw the reflection, we disregard the stencil test
...
void StencilApp::BuildPSOs()
{
	...
	reflectionsDSS.FrontFace.StencilFunc        = D3D12_COMPARISON_FUNC_ALWAYS; //!? look here: we make it so that stencil test passes all the time. In this way, the skull will render as long as it passes the depth test
