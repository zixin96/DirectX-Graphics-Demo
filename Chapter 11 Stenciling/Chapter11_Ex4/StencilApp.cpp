...
void StencilApp::BuildPSOs()
{
	...
	shadowDSS.FrontFace.StencilPassOp      = D3D12_STENCIL_OP_KEEP; //!? look here: originally, we increment the stencil value if stencil and depth test passes so that the second time we draw to the same pixel, stencil test will discard it. In order to show the acene, we will keep the stencil value at 0 all the time, so that stencil test will awlays pass when we draw overlapping triangles
