...
class BoltApp : public D3DApp
{
	public:
		BoltApp(HINSTANCE hInstance);
		BoltApp(const BoltApp& rhs)            = delete;
		BoltApp& operator=(const BoltApp& rhs) = delete;
		~BoltApp() override;
	...
	private:
		...
		void UpdateBoltAnimationFrames(const GameTimer& gt);
	...
	private:
		...
		UINT mBoltFrameIndex = 0; //!? look here: keep track of bolt frame index
...
void BoltApp::UpdateBoltAnimationFrames(const GameTimer& gt)
{
	// animate bolt animation frames
	static float t = 0.0f;
	t += gt.DeltaTime();
	...
	//!? update this to change which texture frame the shader will use
	mMaterials["bolt"]->DiffuseSrvHeapIndex = 2 + mBoltFrameIndex;
}
...
void BoltApp::Draw(const GameTimer& gt)
{
	...
	// draw the bolts last
	mCommandList->SetPipelineState(mPSOs["additiveBlending"].Get());
...
void BoltApp::AnimateMaterials(const GameTimer& gt)
{
	...
	// tweak these parameters will change how texture looks
	auto     boltMat         = mMaterials["bolt"].get();
	XMMATRIX boltScale       = XMMatrixScaling(3.0f, 3.0f, 1.0f);
...
void BoltApp::LoadTextures()
{
	...
	//!? look here:  load 60 frames of bolt animation
	for (int i = 0; i < 60; ++i)
	{
...
void BoltApp::BuildDescriptorHeaps()
{
	...
	srvHeapDesc.NumDescriptors             = (UINT)mTextures.size(); //!? look here:  one descriptor for each texture
	...
	//!? look here:  create descriptors for 60 frames animation
	for (int i = 0; i < 60; i++)
	{
		std::string name = "Bolt";
		if (i + 1 <= 9)
...
void BoltApp::BuildShadersAndInputLayout()
{
	...
	mShaders["standardPS"] = d3dUtil::CompileShader(L"Shaders\\Default.hlsl", nullptr, "PS", "ps_5_0"); //!? look here: to render cylinder bolt, we create a standard pixel shader with no fog (fog will make the cylinder white)
...
void BoltApp::BuildCylinderGeometry()
{
	GeometryGenerator           geoGen;
...
void BoltApp::BuildPSOs()
{
	...
	//
	// PSO for Additive Blending
	// To render bolts, we need 3 things: additive blending, no depth writing, no culling
	//!? Look here
	//
	...
	additivePsoDesc.BlendState.RenderTarget[0].SrcBlendAlpha  = D3D12_BLEND_ZERO;
	additivePsoDesc.BlendState.RenderTarget[0].DestBlendAlpha = D3D12_BLEND_ZERO;
	...
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&additivePsoDesc, IID_PPV_ARGS(&mPSOs["additiveBlending"])));
}
...
void BoltApp::BuildMaterials()
{
	...
	bolt->Name                = "bolt";
	bolt->MatCBIndex          = 2;
	bolt->DiffuseSrvHeapIndex = 2; //!? look here:  we will change this based on which frames we want to render
...
void BoltApp::BuildRenderItems()
{
	...
	auto cyRitem = std::make_unique<RenderItem>();
	XMStoreFloat4x4(&cyRitem->World, XMMatrixTranslation(3.0f, 6.0f, -9.0f));
	cyRitem->ObjCBIndex         = 2;
	cyRitem->Mat                = mMaterials["bolt"].get();
	cyRitem->Geo                = mGeometries["shapeGeo"].get();
	cyRitem->PrimitiveType      = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	cyRitem->IndexCount         = cyRitem->Geo->DrawArgs["cylinder"].IndexCount;
	cyRitem->StartIndexLocation = cyRitem->Geo->DrawArgs["cylinder"].StartIndexLocation;
	cyRitem->BaseVertexLocation = cyRitem->Geo->DrawArgs["cylinder"].BaseVertexLocation;

	mRitemLayer[(int)RenderLayer::AdditiveBlending].push_back(cyRitem.get());

	mAllRitems.push_back(std::move(wavesRitem));
	mAllRitems.push_back(std::move(gridRitem));
	mAllRitems.push_back(std::move(cyRitem));
}
