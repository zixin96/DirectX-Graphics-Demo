...
void LitColumnsApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	//!? Add a point light centered about each sphere above the columns
	int k = 0;
	for (int i = 0; i < 5; i++)
	{
		...
		XMStoreFloat3(&mMainPassCB.Lights[k].Position, XMVector4Transform(XMVectorSet(0.0f, 3.f * sin(gt.TotalTime()), 0.0f, 1.0f), leftSphereWorld));
		mMainPassCB.Lights[k].Strength = {0.6f, 0.6f, 0.6f};
		++k;
		XMStoreFloat3(&mMainPassCB.Lights[k].Position, XMVector4Transform(XMVectorSet(0.0f, 3.f * sin(gt.TotalTime()), 0.0f, 1.0f), rightSphereWorld));
		mMainPassCB.Lights[k].Strength = {0.6f, 0.6f, 0.6f};
		++k;
	}
