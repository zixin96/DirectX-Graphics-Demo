...
BoxMultipleSlotsApp::BoxMultipleSlotsApp(HINSTANCE hInstance)
	: D3DApp(hInstance)
{
}

BoxMultipleSlotsApp::~BoxMultipleSlotsApp()
{
}

bool BoxMultipleSlotsApp::Initialize()
{
...
void BoxMultipleSlotsApp::OnResize()
{
	D3DApp::OnResize();
...
void BoxMultipleSlotsApp::Update(const GameTimer& gt)
{
	OnKeyboardInput(gt);
...
void BoxMultipleSlotsApp::Draw(const GameTimer& gt)
{
	...
	//!? We bind the 2 vertex buffers to slots 0 and 1
	D3D12_VERTEX_BUFFER_VIEW vertexBuffers[] = {mBoxGeo->VertexPosBufferView(), mBoxGeo->VertexColorBufferView()};
	mCommandList->IASetVertexBuffers(0,
	                                 _countof(vertexBuffers), //!? we are binding buffers to input slot 0, 1
	                                 vertexBuffers);
...
void BoxMultipleSlotsApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void BoxMultipleSlotsApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void BoxMultipleSlotsApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void BoxMultipleSlotsApp::OnKeyboardInput(const GameTimer& gt)
{
 ...
 */
void BoxMultipleSlotsApp::BuildDescriptorHeaps()
{
...
void BoxMultipleSlotsApp::BuildCBVs()
{
...
void BoxMultipleSlotsApp::BuildRootSignature()
{
...
void BoxMultipleSlotsApp::BuildInputLayout()
{
	mInputLayout =
	{
		{
			...
			0, //!? Input Slot 0
			0, //!? Note that aligned byte offset is 0, because position and color are no longer interleaved in a single input slot
...
void BoxMultipleSlotsApp::BuildShaders()
{
...
void BoxMultipleSlotsApp::BuildBoxGeometry()
{
	//!? we need to change how we build box geometry
...
void BoxMultipleSlotsApp::BuildPSO()
{
