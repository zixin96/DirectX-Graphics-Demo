...
{
	XMFLOAT3 Pos;
	XMFLOAT4 Color; // added for testing
	XMFLOAT3 Tangent;
	XMFLOAT3 Normal;
	XMFLOAT2 Tex0;
	XMFLOAT2 Tex1;
	XMCOLOR  XMColor; // name has changed
};
	...
	{
		{
			...
			D3D12_APPEND_ALIGNED_ELEMENT,               // 0-byte offset from the start of the C++ Vertex struct of the input slot 0 to the start of the vertex component. could use D3D12_APPEND_ALIGNED_ELEMENT instead of manually specifying the offset
		...
		},
		{
			...
			0,                                          // input slot: this element comes from the input slot 0
			D3D12_APPEND_ALIGNED_ELEMENT,               // 12-byte offset from the start of the C++ Vertex struct of the input slot 0 to the start of the vertex component. could use D3D12_APPEND_ALIGNED_ELEMENT instead of manually specifying the offset
			D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, // Input data is per-vertex data
			0                                           // This value must be 0 for an element that contains per-vertex data
		},
		{
		...
		{
			"XMCOLOR", 0,
			DXGI_FORMAT_B8G8R8A8_UNORM, // NOTE: XMCOLOR has BRGA 8-8-8-8 bit unsigned normalized integer
			0,
			D3D12_APPEND_ALIGNED_ELEMENT,
			D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA,
			0
