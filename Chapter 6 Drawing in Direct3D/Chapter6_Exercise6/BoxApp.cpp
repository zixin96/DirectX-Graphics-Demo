...
void BoxApp::Update(const GameTimer& gt)
{
	...
	PassConstants passConts;
	passConts.gTime = gt.TotalTime();
	mPassCB->CopyData(0, passConts);
}
...
void BoxApp::Draw(const GameTimer& gt)
{
	...
	mCommandList->SetGraphicsRootDescriptorTable(0,              // index of the root parameter we are setting
	                                             cbvHeapHandle); // handle to a descriptor in the heap that specifies the first descriptor in the table being set
...
void BoxApp::BuildDescriptorHeaps()
{
	...
	cbvHeapDesc.NumDescriptors = 2;                                         // one view to describe cbPerObject, one view to describe cbPerPass
...
void BoxApp::BuildConstantBuffers()
{
	...
	md3dDevice->CreateConstantBufferView(&cbvDesc,
	                                     cbvHeapHandle); // Describes the CPU descriptor handle that represents the destination where the newly-created constant buffer view will reside

	cbvHeapHandle.Offset(1, mCbvSrvUavDescriptorSize);

	// create a per-pass constant buffer 
	mPassCB = std::make_unique<UploadBuffer<PassConstants>>(md3dDevice.Get(), 1, true);

	cbAddress = mPassCB->Resource()->GetGPUVirtualAddress();

	cbvDesc.BufferLocation = cbAddress;
	cbvDesc.SizeInBytes    = d3dUtil::CalcConstantBufferByteSize(sizeof(PassConstants)); // must be a multiple of 256
...
void BoxApp::BuildRootSignature()
{
	...
	cbvTablePerObject.Init(D3D12_DESCRIPTOR_RANGE_TYPE_CBV, // table type
	                       1,                               // number of descriptors in the table
	                       0);                              // base shader register arguments are bound to for this root parameter

	CD3DX12_DESCRIPTOR_RANGE cbvTablePerPass;
	cbvTablePerPass.Init(D3D12_DESCRIPTOR_RANGE_TYPE_CBV, // be mindful of copy-paste error! cbvTablePerPass.Init, not cbvTablePerObject.Init 
	                     1,
	                     1);
	...
	slotRootParameter[0].InitAsDescriptorTable(1,                   // number of ranges
	                                           &cbvTablePerObject); // pointer to array of ranges
