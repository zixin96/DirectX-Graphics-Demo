...
VertexOut VS(VertexIn vin)
{
	...
	MaterialData matData = gMaterialData[gObjConstants.gMaterialIndex];
	...
	vout.NormalW = mul(vin.NormalL, (float3x3)gObjConstants.gWorld);

	// Transform to homogeneous clip space.
	vout.PosH = mul(posW, gPassConstants.gViewProj);

	// Output vertex attributes for interpolation across triangle.
	float4 texC = mul(float4(vin.TexC, 0.0f, 1.0f), gObjConstants.gTexTransform);
...
float4 PS(VertexOut pin) : SV_Target
{
	...
	float4      directLight  = ComputeLighting(gPassConstants.gLights,
	                                           mat,
	                                           pin.PosW,
	                                           pin.NormalW,
	                                           toEyeW,
