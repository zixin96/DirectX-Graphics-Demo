...
void BasicTessellationApp::BuildShadersAndInputLayout()
{
	...
	mShaders["tessHS"] = d3dUtil::CompileShader(L"Shaders\\Tessellation.hlsl", nullptr, "HS", "hs_5_0"); // HS is the entry point of a control point null shader that has a field "[patchconstantfunc("ConstantHS")]" that points to constant hull shader
...
/**
 * \brief Create vertex buffer that stores the 3 control points (for tessellating a triangle patch)
 */
void BasicTessellationApp::BuildTrianglePatchGeometry()
{
	...
	geo->DrawArgs["tripatch"] = triSubMesh;
...
/**
 * \brief Build the triangle patch
 */
void BasicTessellationApp::BuildRenderItems()
{
	auto trianglePatchRitem                = std::make_unique<RenderItem>();
	trianglePatchRitem->World              = MathHelper::Identity4x4();
	trianglePatchRitem->TexTransform       = MathHelper::Identity4x4();
	trianglePatchRitem->ObjCBIndex         = 0;
	trianglePatchRitem->Mat                = mMaterials["whiteMat"].get();
	trianglePatchRitem->Geo                = mGeometries["trianglepatchGeo"].get();
	trianglePatchRitem->PrimitiveType      = D3D_PRIMITIVE_TOPOLOGY_3_CONTROL_POINT_PATCHLIST; // a patch with 3 control points
	trianglePatchRitem->IndexCount         = trianglePatchRitem->Geo->DrawArgs["tripatch"].IndexCount;
	trianglePatchRitem->StartIndexLocation = trianglePatchRitem->Geo->DrawArgs["tripatch"].StartIndexLocation;
	trianglePatchRitem->BaseVertexLocation = trianglePatchRitem->Geo->DrawArgs["tripatch"].BaseVertexLocation;
	mRitemLayer[(int)RenderLayer::Opaque].push_back(trianglePatchRitem.get());

	mAllRitems.push_back(std::move(trianglePatchRitem));
}
