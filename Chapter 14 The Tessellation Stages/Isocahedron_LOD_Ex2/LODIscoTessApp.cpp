#include "LODIscoTessApp.h"

using Microsoft::WRL::ComPtr;
using namespace DirectX;
using namespace PackedVector;

LODIscoTessApp::LODIscoTessApp(HINSTANCE hInstance)
	: D3DApp(hInstance)
{
}

LODIscoTessApp::~LODIscoTessApp()
{
	if (md3dDevice != nullptr)
		FlushCommandQueue();
}

bool LODIscoTessApp::Initialize()
{
...
void LODIscoTessApp::Update(const GameTimer& gt)
{
	OnKeyboardInput(gt);
	UpdateCamera(gt);
...
void LODIscoTessApp::Draw(const GameTimer& gt)
{
...
void LODIscoTessApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void LODIscoTessApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void LODIscoTessApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void LODIscoTessApp::OnKeyboardInput(const GameTimer& gt)
{
...
void LODIscoTessApp::UpdateCamera(const GameTimer& gt)
{
...
void LODIscoTessApp::UpdateObjectCBs(const GameTimer& gt)
{
...
void LODIscoTessApp::UpdateMainPassCB(const GameTimer& gt)
{
...
// See Notability
void LODIscoTessApp::BuildDescriptorHeaps()
{
	//
	// Create the CBV heap.
	//
...
// See Notability
void LODIscoTessApp::BuildDescriptors()
{
	UINT objCount      = (UINT)mOpaqueRitems.size();
...
void LODIscoTessApp::BuildRootSignature()
{
...
void LODIscoTessApp::BuildShadersAndInputLayout()
{
	mShaders["standardVS"] = d3dUtil::CompileShader(L"Shaders\\color.hlsl", nullptr, "VS", "vs_5_1");
	//!? add HS stage
	mShaders["standardHS"] = d3dUtil::CompileShader(L"Shaders\\color.hlsl", nullptr, "HS", "hs_5_1");
	//!? add DS stage
	mShaders["standardDS"] = d3dUtil::CompileShader(L"Shaders\\color.hlsl", nullptr, "DS", "ds_5_1");
...
void LODIscoTessApp::BuildShapeGeometry()
{
	// we place all scene geometry in one big vertex and index buffer

	GeometryGenerator           geoGen;
	GeometryGenerator::MeshData sphere = geoGen.CreateGeosphere(mSphereRadius, 0);
...
void LODIscoTessApp::BuildPSOs()
{
	...
	opaquePsoDesc.PrimitiveTopologyType    = D3D12_PRIMITIVE_TOPOLOGY_TYPE_PATCH; //!? change topology type to patch 
...
void LODIscoTessApp::BuildFrameResources()
{
...
void LODIscoTessApp::BuildRenderItems()
{
	...
	geoSphere->Radius             = mSphereRadius;
	geoSphere->PrimitiveType      = D3D_PRIMITIVE_TOPOLOGY_3_CONTROL_POINT_PATCHLIST;
...
void LODIscoTessApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
