...
class ConsumeAppendBufferCSApp : public D3DApp
{
public:
	ConsumeAppendBufferCSApp(HINSTANCE hInstance);
	ConsumeAppendBufferCSApp(const ConsumeAppendBufferCSApp& rhs)            = delete;
	ConsumeAppendBufferCSApp& operator=(const ConsumeAppendBufferCSApp& rhs) = delete;
	~ConsumeAppendBufferCSApp() override;
                   ...
                   PSTR      cmdLine, int         showCmd)
{
	...
	try
	{
		ConsumeAppendBufferCSApp theApp(hInstance);
...
ConsumeAppendBufferCSApp::ConsumeAppendBufferCSApp(HINSTANCE hInstance)
	: D3DApp(hInstance)
{
}

ConsumeAppendBufferCSApp::~ConsumeAppendBufferCSApp()
{
	if (md3dDevice != nullptr)
		FlushCommandQueue();
}

bool ConsumeAppendBufferCSApp::Initialize()
{
...
void ConsumeAppendBufferCSApp::Update(const GameTimer& gt)
{
...
void ConsumeAppendBufferCSApp::Draw(const GameTimer& gt)
{
...
void ConsumeAppendBufferCSApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void ConsumeAppendBufferCSApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void ConsumeAppendBufferCSApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void ConsumeAppendBufferCSApp::DoComputeWork()
{
	// Reuse the memory associated with command recording.
	// We can only reset when the associated command lists have finished execution on the GPU.
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> ConsumeAppendBufferCSApp::GetStaticSamplers()
{
