...
class StructuredBufferCSApp : public D3DApp
{
public:
	StructuredBufferCSApp(HINSTANCE hInstance);
	StructuredBufferCSApp(const StructuredBufferCSApp& rhs)            = delete;
	StructuredBufferCSApp& operator=(const StructuredBufferCSApp& rhs) = delete;
	~StructuredBufferCSApp() override;
                   ...
                   PSTR      cmdLine, int         showCmd)
{
	...
	try
	{
		StructuredBufferCSApp theApp(hInstance);
...
StructuredBufferCSApp::StructuredBufferCSApp(HINSTANCE hInstance)
	: D3DApp(hInstance)
{
}

StructuredBufferCSApp::~StructuredBufferCSApp()
{
	if (md3dDevice != nullptr)
		FlushCommandQueue();
}

bool StructuredBufferCSApp::Initialize()
{
...
void StructuredBufferCSApp::OnResize()
{
	D3DApp::OnResize();
...
void StructuredBufferCSApp::Update(const GameTimer& gt)
{
...
void StructuredBufferCSApp::Draw(const GameTimer& gt)
{
...
void StructuredBufferCSApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void StructuredBufferCSApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void StructuredBufferCSApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void StructuredBufferCSApp::DoComputeWork()
{
	// Reuse the memory associated with command recording.
	// We can only reset when the associated command lists have finished execution on the GPU.
...
void StructuredBufferCSApp::BuildBuffers()
{
	// Generate some data.
...
void StructuredBufferCSApp::BuildRootSignature()
{
...
void StructuredBufferCSApp::BuildShadersAndInputLayout()
{
	mShaders["StructBufferCS"] = d3dUtil::CompileShader(L"Shaders\\StructuredBuffer.hlsl", nullptr, "CS", "cs_5_0");
}

void StructuredBufferCSApp::BuildPSOs()
{
...
void StructuredBufferCSApp::BuildFrameResources()
{
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> StructuredBufferCSApp::GetStaticSamplers()
{
